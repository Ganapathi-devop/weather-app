...
function setApi(position) {
  ...
  async function getweatherData(url) {
    ...
    console.log(elementsOfWeatherDats);

    if (elementsOfWeatherDats.includes("daily") ) {
      $(".hidden-div").hide();
      $(".div-wrapper").show();
      var sunrise =
        weatherData.daily.sunrise[weatherData.daily.sunrise.length - 1];
      $("#sunriseTime").html(unixToTime(sunrise));
      let sunset =
        weatherData.daily.sunset[weatherData.daily.sunset.length - 1];
  ...
  $(".searchSide-btn").on("click", () => {
    ...
    async function getLocationData(url) {
      ...
      let searched_lat_long =
        loc_data.response.GeoObjectCollection.featureMember[0].GeoObject.Point
          .pos;
  ...
  $(".btn-cast-yesterday, .btn-cast-tomorrow, .btn-cast-today").click((p) => {
    let input_day = p.target.innerText;
    let btn = p.target.classList
    $(".btn").removeClass("active btn-warning")
    btn.add("active", "btn-warning")
    if (input_day === "Yesterday") {
      getweatherData(`https://api.open-meteo.com/v1/forecast?latitude=${position.coords.latitude}&longitude=${position.coords.longitude}&daily=weathercode,temperature_2m_max,temperature_2m_min,sunrise,sunset,windspeed_10m_max,windgusts_10m_max&current_weather=true&timeformat=unixtime&timezone=Asia%2FBangkok&past_days=1`);
    } else if (input_day === "Tomorrow") {
      let date = new Date();
      let mon = date.getMonth()+1 >9? date.getMonth()+1 : `0${date.getMonth()+1}`
      let today = `${date.getFullYear()}-${mon}-${date.getDate()}`;
      let tomorrow = `${date.getFullYear()}-${mon}-${date.getDate() + 1}`;
      getweatherData(
        `${weatherApi_url}&start_date=${today}&end_date=${tomorrow}`
      );
    }else{
        getweatherData(weatherApi_url)
    }
